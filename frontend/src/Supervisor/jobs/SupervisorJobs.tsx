// Supervisor/jobs/SupervisorJobs.tsx
// Full job CRUD for supervisors, company-wide — mirrors Admin's job manager
// but with no Purchase Order / budget fields in the form. The backend
// auto-selects/handles PO allocation for jobs created by a supervisor.
...
interface Job {
  ...
  venue: string; address?: string; city?: string; lat?: number; lng?: number
...
const EMPTY_FORM = {
  ...
  city: '', date: '', endDate: '', startTime: '09:00', endTime: '17:00',
  hourlyRate: '', totalSlots: '4', status: 'OPEN', filters: {} as any,
  termsAndConditions: '',
}

const STATUS_OPTS = ['OPEN', 'FILLED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']

export const SupervisorJobs: React.FC = () => {
  ...
  useEffect(() => {
    if (modal === 'edit' && editing) {
      setForm({
        title: editing.title, client: editing.client, clientId: editing.clientId || '',
        brand: editing.brand || '', venue: editing.venue || '', city: editing.city || '',
        date: editing.date ? editing.date.slice(0, 10) : '', endDate: editing.endDate ? editing.endDate.slice(0, 10) : '',
        startTime: editing.startTime || '09:00', endTime: editing.endTime || '17:00',
        hourlyRate: String(editing.hourlyRate || ''), totalSlots: String(editing.totalSlots || 4),
        status: editing.status || 'OPEN', filters: editing.filters || {},
  ...
  return (
    <div style={{ padding: '32px 36px 80px' }}>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: GL, marginBottom: 8, fontWeight: 700, fontFamily: FD }}>Operations</div>
        <h1 style={{ fontFamily: FD, fontSize: 26, fontWeight: 700, color: W }}>Manage Jobs</h1>
        <p style={{ fontSize: 12.5, color: WM, marginTop: 4 }}>{jobs.length} total · {jobs.filter(j => j.status === 'OPEN').length} open</p>
      </div>
      {/* Stats */}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <input placeholder="Search jobs or client…" value={search} onChange={e => setSearch(e.target.value)}
            style={{ background: 'rgba(248,248,248,0.05)', border: `1px solid ${BB}`, padding: '9px 14px', color: W, fontFamily: FB, fontSize: 13, outline: 'none', borderRadius: 2, maxWidth: 240 }}
      ...
      <div style={{ background: D2, border: `1px solid ${BB}`, borderRadius: 2, overflow: 'hidden' }}>
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            ...
            <tbody>
              {jobs.filter(j => (statusF === 'all' || j.status === statusF) && (!search || j.title.toLowerCase().includes(search.toLowerCase()) || j.client.toLowerCase().includes(search.toLowerCase()))).map((job, i, arr) => (
                <tr key={job.id} style={{ borderBottom: i < arr.length - 1 ? `1px solid ${BB}` : 'none' }}
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: job.status === 'OPEN' ? GL : job.status === 'FILLED' ? G : job.status === 'IN_PROGRESS' ? TEAL : job.status === 'CANCELLED' ? CORAL : WD, background: 'rgba(255,255,255,0.05)', padding: '3px 9px', borderRadius: 2 }}>{job.status}</span>
                  </td>
                  <td style={{ padding: '12px 14px' }}>
        ...
        {!loading && jobs.length === 0 && <div style={{ padding: 48, textAlign: 'center', color: WD, fontSize: 13 }}>No jobs yet.</div>}
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400, padding: 16 }}>
          <div style={{ background: BC, border: `1px solid ${CORAL}`, padding: '28px 24px', maxWidth: 360, width: '100%', borderRadius: 3 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={async () => { try { await fetch(`${API}/jobs/${deleting}`, { method: 'DELETE', headers: authHdr() as any }); await loadJobs(); setDeleting(null) } catch { setError('Failed to delete job.') } }}
      ...
      {/* Create / Edit modal — NO Purchase Order fields; backend auto-handles PO */}
          ...
          <div style={{ background: BC, border: `1px solid ${BB}`, width: '100%', maxWidth: 560, maxHeight: '92vh', overflowY: 'auto', position: 'relative', borderRadius: 3 }}>
            ...
            <div style={{ padding: '32px 24px 28px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  <label style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: WM, display: 'block', marginBottom: 7 }}>Client *</label>
                  <select style={{ width: '100%', background: D3, border: `1px solid ${BB}`, padding: '10px 14px', color: W, fontFamily: FB, fontSize: 13, outline: 'none', borderRadius: 2, cursor: 'pointer' }}
                    value={form.client} onChange={e => { const name = e.target.value; const found = clients.find(c => c.name === name); setForm(f => ({ ...f, client: name, clientId: found?.id || '' })) }}>
                ...
                <button onClick={async () => {
                  ...
                  const body: any = {
                    ...
                    totalSlots: Number(form.totalSlots) || 1, status: form.status, filters: form.filters || {},
