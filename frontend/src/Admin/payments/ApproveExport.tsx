      ...
      <div style={{ padding: '40px 48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 32 }}>
          ...
          <ExportCard
            ...
            onCSV={() => { exportCSV([payrollHeaders, ...payrollRows(MOCK_RECORDS)] as string[][], `honeygroup-promotions-payroll-${today()}.csv`); flash('Payroll CSV downloaded') }}
            onExcel={() => { exportExcel(payrollHeaders, payrollRows(MOCK_RECORDS), `honeygroup-promotions-payroll-${today()}.xls`); flash('Payroll Excel downloaded') }}
            onPDF={() => { exportPDF(buildCampaignPDFHtml(MOCK_RECORDS), `honeygroup-promotions-payroll-${today()}`); flash('Payroll PDF export initiated') }}
          />

          <ExportCard
            icon="📋" title="Campaign Client Report" color={GL}
            description="Per-client attendance and payout summary grouped by client."
            onCSV={() => { exportCSV([payrollHeaders, ...payrollRows(MOCK_RECORDS)] as string[][], `honeygroup-promotions-campaign-${today()}.csv`); flash('Campaign CSV downloaded') }}
            onExcel={() => { exportExcel(payrollHeaders, payrollRows(MOCK_RECORDS), `honeygroup-promotions-campaign-${today()}.xls`); flash('Campaign Excel downloaded') }}
            onPDF={() => { exportPDF(buildCampaignPDFHtml(MOCK_RECORDS), `honeygroup-promotions-campaign-${today()}`); flash('Campaign PDF export initiated — check your browser print dialog') }}
          />

          <ExportCard
            icon="💼" title="Jobs Register" color={G4}
            description="All active and archived jobs with slots, pay rates, locations, and status."
            onCSV={() => { const d = buildJobsData(); if (d.empty) { flash('No jobs data available'); return }; exportCSV([d.headers, ...d.rows] as string[][], `honeygroup-promotions-jobs-${today()}.csv`); flash('Jobs CSV downloaded') }}
            onExcel={() => { const d = buildJobsData(); if (d.empty) { flash('No jobs data available'); return }; exportExcel(d.headers, d.rows, `honeygroup-promotions-jobs-${today()}.xls`); flash('Jobs Excel downloaded') }}
            onPDF={() => { const d = buildJobsData(); if (d.empty) { flash('No jobs data available'); return }; const trs = d.rows.map((r: string[]) => `<tr>${r.map((c: string) => `<td>${c}</td>`).join('')}</tr>`).join(''); exportPDF(`<style>body{font-family:Georgia;background:#050504;color:#F8F8F8;padding:40px}table{width:100%;border-collapse:collapse}th{background:rgba(170,160,135,0.2);padding:8px;font-size:10px;letter-spacing:0.15em;text-transform:uppercase;color:#C9BFA6}td{padding:8px;border-bottom:1px solid rgba(170,160,135,0.12);font-size:12px;color:rgba(248,248,248,0.85)}</style><h2 style="color:#C9BFA6;font-family:Georgia;margin-bottom:24px">Jobs Register</h2><table><thead><tr>${d.headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${trs}</tbody></table>`, `honeygroup-promotions-jobs-${today()}`); flash('Jobs PDF export initiated') }}
          />

          <ExportCard
            icon="👥" title="Promoter Roster" color={G3}
            description="Full promoter list with city, reliability scores, and onboarding status."
            onCSV={() => { const d = buildPromotersData(); if (d.empty) { flash('No promoter data available'); return }; exportCSV([d.headers, ...d.rows] as string[][], `honeygroup-promotions-promoters-${today()}.csv`); flash('Promoters CSV downloaded') }}
            onExcel={() => { const d = buildPromotersData(); if (d.empty) { flash('No promoter data available'); return }; exportExcel(d.headers, d.rows, `honeygroup-promotions-promoters-${today()}.xls`); flash('Promoters Excel downloaded') }}
            onPDF={() => { const d = buildPromotersData(); if (d.empty) { flash('No promoter data available'); return }; const trs = d.rows.map((r: string[]) => `<tr>${r.map((c: string)=>`<td>${c}</td>`).join('')}</tr>`).join(''); exportPDF(`<style>body{font-family:Georgia;background:#050504;color:#F8F8F8;padding:40px}table{width:100%;border-collapse:collapse}th{background:rgba(170,160,135,0.2);padding:8px;font-size:10px;letter-spacing:0.15em;text-transform:uppercase;color:#C9BFA6}td{padding:8px;border-bottom:1px solid rgba(170,160,135,0.12);font-size:12px;color:rgba(248,248,248,0.85)}</style><h2 style="color:#C9BFA6;font-family:Georgia;margin-bottom:24px">Promoter Roster</h2><table><thead><tr>${d.headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${trs}</tbody></table>`, `honeygroup-promotions-promoters-${today()}`); flash('Promoters PDF export initiated') }}
          />
          ...
          <ExportCard
            ...
            onCSV={() => {
              ...
              exportCSV([headers, ...rows] as string[][], `honeygroup-promotions-eft-batch-${today()}.csv`)
            ...
            onExcel={() => {
              ...
              const headers = ['Promoter','Email','Bank','Account No','Net Payout (R)','Job','Date']
              const rows    = approved.map(r => [r.promoter, r.email, r.bank, r.accountNo, net(r), r.job, r.date])
              exportExcel(headers, rows, `honeygroup-promotions-eft-batch-${today()}.xls`)
          ...
          <ExportCard
            ...
            onCSV={() => { exportCSV([attendanceHeaders, ...attendanceRows(MOCK_RECORDS)] as string[][], `honeygroup-promotions-attendance-${today()}.csv`); flash('Attendance CSV downloaded') }}
            onExcel={() => { exportExcel(attendanceHeaders, attendanceRows(MOCK_RECORDS), `honeygroup-promotions-attendance-${today()}.xls`); flash('Attendance Excel downloaded') }}
            onPDF={() => {
              const trs = attendanceRows(MOCK_RECORDS).map(r => `<tr>${r.map(c=>`<td>${c}</td>`).join('')}</tr>`).join('')
              exportPDF(`<style>body{font-family:Georgia;background:#050504;color:#F8F8F8;padding:40px}table{width:100%;border-collapse:collapse}th{background:rgba(170,160,135,0.2);padding:8px;font-size:10px;letter-spacing:0.15em;text-transform:uppercase;color:#C9BFA6}td{padding:8px;border-bottom:1px solid rgba(170,160,135,0.12);font-size:12px;color:rgba(248,248,248,0.85)}</style><h2 style="color:#C9BFA6;font-family:Georgia;margin-bottom:24px">Attendance Summary</h2><table><thead><tr>${attendanceHeaders.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${trs}</tbody></table>`, `honeygroup-promotions-attendance-${today()}`)
        ...
        <div style={{ background: D2, border: `1px solid ${BB}`, borderRadius: 4, padding: '28px 32px' }}>
          ...
          <div style={{ marginTop: 20, display: 'flex', gap: 10 }}>
            <button onClick={() => {
              ...
              exportCSV([headers,...rows], `honeygroup-promotions-payout-estimate-${today()}.csv`); flash('Payout estimate CSV downloaded')
            ...
            <button onClick={() => {
              ...
              exportExcel(headers, rows, `honeygroup-promotions-payout-estimate-${today()}.xls`); flash('Payout estimate Excel downloaded')
