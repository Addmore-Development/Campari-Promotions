...
export default function RegisterPage() {
  ...
  if (validateStep()) {
    ...
    setTimeout(() => {
      document.getElementById('reg-error-summary')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }, 50)
  }
}
      ...
      <div className="hg-reg" style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 560 }}>
        ...
        <div style={{ background: BLACK_CARD, border: `1px solid ${BLACK_BORDER}`, padding: '40px 40px 36px', position: 'relative', boxShadow: '0 40px 100px rgba(0,0,0,0.6)' }}>
          ...
          {Object.keys(errors).length > 0 && (
            <div id="reg-error-summary" style={{ marginBottom: 24, padding: '14px 18px', background: `${AMBER}18`, border: `1px solid ${AMBER}55` }}>
              <p style={{ fontFamily: FB, fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: AMBER, marginBottom: 8 }}>
                Please fix the following before continuing:
              </p>
              <ul style={{ margin: 0, paddingLeft: 18 }}>
                {Object.entries(errors).map(([key, msg]) => (
                  <li key={key} style={{ fontFamily: FB, fontSize: 12, color: WHITE_MUTED, marginBottom: 3 }}>{msg}</li>
                ))}
              </ul>
            </div>
          )}
